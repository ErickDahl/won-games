...
const authClasses = tv({
  slots: {
    ...
    leftDivContent: 'relative z-10 flex h-screen flex-col items-start justify-between p-8',
    ...
    rightDiv: 'flex h-screen w-full flex-col items-center justify-center bg-white px-[10%] md:w-1/2 lg:p-8 lg:px-[10%]',
    rightDivLogo: 'mb-[10%]',
...
const RenderActionText = ({ pageType }: Pick<AuthTemplateProps, 'pageType'>) => {
  const { rightDivActionText } = authClasses()

  const isSignup = pageType === 'signup'
  const actionText = isSignup ? 'Already have an account' : 'Don’t have an account?'
  ...
  return (
    <p className={rightDivActionText()}>
      {actionText}{' '}
      <StoreLink className="text-line text-sm text-secondary underline" hoverEffect={false} href={linkHref}>
        {linkText}
      </StoreLink>
    </p>
  )
...
const AuthTemplate = ({ pageType, className, children }: PropsWithChildren<AuthTemplateProps>) => {
  const {
    base,
    leftDiv,
    rightDiv,
    ...
    <main className={base({ className })}>
        ...
        <div className={leftDiv()} style={{ backgroundImage: `url(${backgroundImage.src})` }}>
          <div className={leftDivContent()}>
            ...
            <div className={leftDivContentText()}>
              <Heading className={leftDivContentTextHeading()}>All your favorite games in one place</Heading>
              <h3 className={leftDivContentSubTitle()}>
                <strong className={strongClass()}>WON</strong> is the best and most complete gaming platform.
              </h3>
            </div>
            <p className={footerText()}>Won Games 2020 © Todos os Direitos Reservados</p>
          </div>
        </div>
      )}
      ...
      <div className={rightDiv()}>
        ...
        <div className={rightDivContent()}>
          <Heading className="m-0" color="black" lineColor="secondary" linePosition="left">
