...
const gameCardCarouselClasses = tv({
  slots: {
    ...
    carouselItemClass: 'mx-3 flex w-full flex-[0_0_70%] items-center justify-center lg:flex-[0_0_33%]'
  }
})
    ...
    <Carousel className={base()} opts={{ align: 'start' }} {...rest}>
      <CarouselContent>
        {cards?.map((card, index) => (
          <CarouselItem className={carouselItemClass()} index={index} key={uuidv4()}>
            <GameCard {...card} />
          </CarouselItem>
        ))}
      </CarouselContent>
